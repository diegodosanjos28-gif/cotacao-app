...
export default function HallFornecedorCard(props: Props) {
  ...
  if (isAtiva) {
    ...
    return (
      <div
        className={`flex flex-col gap-3 rounded-2xl border p-4 transition-all hover:-translate-y-[3px] hover:shadow-[0_14px_30px_rgba(37,34,32,.12)] ${
          respondeu
      ...
      >
        <div className="flex items-center gap-3">
          <span
            className={`flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-full text-sm font-medium text-white shadow-[0_2px_8px_rgba(0,0,0,.16)] ${respondeu ? "" : "opacity-55"}`}
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-0.5 flex items-center gap-1.5 text-[11px] font-normal text-t3">
              <ClockIcon />
              {respondeu ? `Respondeu ${formatarTempoRelativo(fornecedor.respondeuEm)}` : "Ainda não respondeu"}
            </div>
          </div>
          <span
            className={`ml-auto inline-flex shrink-0 items-center gap-1.5 self-start whitespace-nowrap rounded-full px-2.5 py-1 text-[10px] font-medium ${
        ...
        <div className="flex flex-col justify-center gap-1.5">
          <div className="flex items-baseline justify-between gap-2">
            <span className="text-[9px] font-medium uppercase tracking-wide text-t3">Cobertura da lista</span>
            <span className="font-[var(--font-sans)] text-[11.5px] font-normal text-t2">
              <b className="font-light text-[19px] tracking-[-.6px] text-prx-l">{respondeu ? fornecedor.itensCotados : "—"}</b> / {itensListaBase} itens
            </span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-t1/[.08]">
            <span
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-bdr bg-card p-4 shadow-[0_6px_20px_rgba(37,34,32,.07),0_1px_3px_rgba(37,34,32,.05)] transition-all hover:-translate-y-[3px] hover:shadow-[0_14px_30px_rgba(37,34,32,.12)]">
      <div className="flex items-center gap-3">
        <span className="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-full text-sm font-medium text-white shadow-[0_2px_8px_rgba(0,0,0,.16)]" style={{ background: cor }}>
          {iniciais(nome)}
        </span>
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm font-medium text-t1">{nome}</div>
        ...
        <span className={`ml-auto inline-flex shrink-0 items-center gap-1.5 self-start whitespace-nowrap rounded-full border px-2.5 py-1 text-[10px] font-medium ${seloClasse}`}>
      ...
      <div className="flex flex-col justify-center gap-1.5">
        <div className="flex items-baseline justify-between gap-2">
          <span className="text-[9px] font-medium uppercase tracking-wide text-t3">Cobertura média</span>
          <span className="font-[var(--font-sans)] text-[11.5px] font-normal text-t2">
            <b className="font-light text-[19px] tracking-[-.6px] text-prx-l">{Math.round(fornecedor.coberturaMediaPct)}</b>%
          </span>
        </div>
        <div className="h-1.5 overflow-hidden rounded-full bg-t1/[.08]">
      ...
      <div className="flex gap-2">
        <div className="flex min-w-0 flex-1 flex-col justify-center rounded-[10px] border border-bdr bg-surf px-2.5 py-1.5">
          <div className="flex items-center gap-1 text-[8px] font-medium uppercase tracking-wide text-t3">
            <DocIcon />
            Cotações
          </div>
          <div className="mt-0.5 whitespace-nowrap text-xs font-medium text-t1">{fornecedor.cotacoesParticipadas}</div>
        </div>
        <div className="flex min-w-0 flex-1 flex-col justify-center rounded-[10px] border border-bdr bg-surf px-2.5 py-1.5">
          <div className="flex items-center gap-1 text-[8px] font-medium uppercase tracking-wide text-t3">
            <ClockIcon />
          ...
          <div className="mt-0.5 whitespace-nowrap text-xs font-medium text-t1">{formatarDuracaoMinutos(fornecedor.tempoRespostaMedioMinutos)}</div>
        </div>
      </div>
    </div>
  );
